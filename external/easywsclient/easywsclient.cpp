...
{
  public:
    void poll(int timeout) { UNUSED(timeout) }
    void send(const std::string& message) { UNUSED(message) }
    void sendBinary(const std::string& message) { UNUSED(message) }
    void sendBinary(const std::vector<uint8_t>& message) { UNUSED(message) }
    void sendPing() { }
    void close() { }
    readyStateValues getReadyState() const { return CLOSED; }
    void _dispatch(Callback_Imp & callable) { UNUSED(callable) }
    void _dispatchBinary(BytesCallback_Imp& callable) { UNUSED(callable) }
};
...
class _RealWebSocket : public easywsclient::WebSocket
{
  public:
    ...
    void poll(int timeout) { // timeout in milliseconds
        ...
        while (txbuf.size()) {
            int ret = (int)::send(sockfd, (char*)&txbuf[0], txbuf.size(), 0);
            if (false) { } // ??
            else if (ret < 0 && (socketerrno == SOCKET_EWOULDBLOCK || socketerrno == SOCKET_EAGAIN_EINPROGRESS)) {
                break;
            }
