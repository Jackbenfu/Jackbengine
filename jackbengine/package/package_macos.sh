...
if [[ ! "$1" == /* ]]; then
  printText "Project path ($1) must be absolute!"
  exit
fi

bundle_dir=$(echo "$1" | sed 's/\/*$//g')
bundle_name=$2
...
printTitle "Copying executable into target bundle"
function copyExecutable() {
  printText "  Copying ""${bundle_name}"""

  mkdir -p "${bundle_dir}"/"${bundle_target_dir}"
  cp "${bundle_dir}"/"${bundle_name}" "${bundle_dir}"/"${bundle_target_dir}"
}
copyExecutable
