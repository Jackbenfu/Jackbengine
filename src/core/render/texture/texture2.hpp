...
public:
    ...
    Texture2(const Renderer2& renderer, int width, int height, Color color);
    ...
    Texture2(const Renderer2& renderer, const TmxMap& map, const TmxObjectGroup& objectGroup,
             const void *tilesetImageData, size_t tilesetImageDataSize);
    Texture2(const Renderer2& renderer, const Font2& font, const std::string& text, Color foreground);
