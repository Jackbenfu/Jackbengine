...
#include <limits>
#include "aabbCollisionSystem2.hpp"
...
void AABBCollisionSystem2::frame(float delta)
{
...
bool AABBCollisionSystem2::hasRequiredComponents(ComponentCollection& components) const
{
...
void AABBCollisionSystem2::addGroup(const std::string& tag1, const std::string& tag2)
{
...
void AABBCollisionSystem2::removeGroup(const std::string& tag1, const std::string& tag2)
{
    const auto predicate = [&](const auto& pair)
    {
        return pair.first == tag1 && pair.second == tag2;
...
void AABBCollisionSystem2::setCallback(AABBCollisionCallback2 callback)
{
    m_callback = callback;
}

void AABBCollisionSystem2::testCollision(float delta, ComponentCollection& components1, ComponentCollection& components2) const
{
    ...
    if (!(x1 >= x2 + w2 || x1 + w1 <= x2 || y1 >= y2 + h2 || y1 + h1 <= y2))
    {
        ...
        if (xOffsetToCollision > yOffsetToCollision)
        {
            ...
            collisionSide = -1.0f == normalX ? AABBCollisionSide2::Left : AABBCollisionSide2::Right;
        }
        else
        {
            ...
            collisionSide = -1.0f == normalY ? AABBCollisionSide2::Top : AABBCollisionSide2::Bottom;
        }
